		...
		int start_line)
{
	...
	if (map->height > CARBON_MAX_ROWS)
	{
		printf("Error: Map height (%d) exceeds CARBON_MAX_ROWS (%d)\n",
			map->height, CARBON_MAX_ROWS);
		return (0);
	}
	map->width = find_max_width(data->file_content, start_line);
	if (map->width == 0 || map->width > CARBON_MAX_COLUMS)
	{
		printf("Error: Map width (%d) invalid or exceeds "
			"CARBON_MAX_COLUMS (%d)\n", map->width, CARBON_MAX_COLUMS);
		return (0);
	}
	return (1);
}
